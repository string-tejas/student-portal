...
const NewsPage = () => {
    ...
    useEffect(() => {
        fetch('https://newsapi.org/v2/top-headlines?country=in&category=business&apiKey=43551de3a3274f8196dca4b37d12e012') 
          .then((response) => response.json())
          .then((data) => {
    ...
    return <div>
      ...
      <div style={{display:"flex"}}>
        ...
        <div style={{marginBottom:"1rem"}} key={index} className="col-span-8 md:col-span-2 h-50 bg-gray-800 rounded-lg border border-gray-700 px-3 py-3">
        ...
        <div style={{marginBottom:"1rem"}} key={index} className="col-span-6 md:col-span-2 h-50 bg-gray-800 rounded-lg border border-gray-700 px-3 py-3">
            <h1 style={{fontSize:"1.5rem", fontWeight:"bolder"}}>{article.title}</h1>
            <p>{article.description}</p>
            <a href={article.url} target="_blank" rel="noopener noreferrer">
              Read more
        ...
        <div style={{marginLeft:"1rem"}}>
          <h1 style={{fontSize:"2rem"}}>Business</h1>
          <hr />
          <br />
